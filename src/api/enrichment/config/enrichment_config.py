...
class EnrichmentConfig(object):
    ...
    def __init__(self):
        ...
        self._azure_computer_vision_endpoint = os.environ.get(
    ...
    def vision_key(self) -> str:
        if not self._azure_computer_vision_key:
            self._azure_computer_vision_key = os.environ.get(
    ...
    def enrichment_cache_max_expiry_in_sec(self) -> int:
        if not self._enrichment_cache_max_expiry_in_sec:
            try:
                self._enrichment_cache_max_expiry_in_sec = int(
                    float(
                        os.environ.get(
                            "ENRICHMENT_CACHE_MAX_EXPIRY_IN_SEC",
                            DEFAULT_TTL_FOR_CACHING,
                        )
                    )
                )
            except:
            ...
            if (
                self._enrichment_cache_max_expiry_in_sec == 0
                or self._enrichment_cache_max_expiry_in_sec < -1
            ):
                raise ValueError(
                    "ENRICHMENT_CACHE_MAX_EXPIRY_IN_SEC is not defined or Invalid. ENRICHMENT_CACHE_MAX_EXPIRY_IN_SEC can be either a positive number or -1"
                )
    ...
    def cosmos_collection_name(self) -> str:
        if not self._cosmos_collection_name:
            self._cosmos_collection_name = os.environ.get(
                "AZURE_COSMOS_DB_ENRICHMENT_CONTAINER"
            )
